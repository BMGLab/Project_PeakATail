...
| 16.2 | Main manuscript (title page → declarations → references → figure legends) | `PI ACTION` | Source is Markdown; the system expects a word-processor or TeX/PDF file `[INFERRED]`. Needs a conversion pass, with the Abstract rewritten per §2, the abbreviations/order fixes of §1, and the reference list generated from `submission/references.bib` per §7.2. |
