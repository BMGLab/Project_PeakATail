...
- **Stage-3 replication — VERIFIED FIXED (v1 code; SUPERSEDED 2026-08-22 by the v2 record below)**
  ([20_stage3_replication.md](20_stage3_replication.md)): primary
  ...
  regenerated by Stage-3 v2; retained in `20` only as the labelled comparison row. Disclosures: top-gene names
  unreliable in overlapping loci (issue #99: CD68 etc.); MetBone premise wrong (clip-rate sampling bug, same
  issue); null floor 1/11.
...
- **Stage-3 v2 replication — VERIFIED SOUND 2026-08-22, NOW THE STAGE-3 RECORD**
  ([20_stage3_replication.md](20_stage3_replication.md), code 9dfdefb3, root `stage3_laughney_v3`): universe
  80,464 PAS (v1 74,954: net +5,510 = 6,391 minus-strand admitted, 881 newly IP-flagged); primary (MetBone
  excluded, 15 libraries / 12 patients): **15,942 replicated (pair,PAS) at K=2** (15,212 with effect floor) =
  5,951 PAS / 2,883 genes / 47 pairs; 10,415 (pair,gene) combinations; K=3 5,438; sensitivity (13 patients)
  16,010; **0 replicated in all 10 null combos in all four configs of both arms** (58.9M null tests → 11 nominal
  q<0.05, none twice — "none in 10 nulls, empirical p ≤ 0.091", never an FDR). The +1,462 replicated PAS are NOT
  all universe growth: only net +744 involve PAS whose universe membership changed. Disclosures re-checked on v2
  and unchanged in kind: gene names unreliable (12/30 top rows, issue #99), MetBone premise still wrong (306k clip
  molecules; head-sampling warning fired on 5 libraries), GSM3516672 still trips the perms-with-hit count rule via
  BH discreteness. Fig 6 re-rendered from the v3 tree (`LAUGHNEY_SWITCHES_VERSION=v2_code`, now the default).
