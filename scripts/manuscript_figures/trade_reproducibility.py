#!/usr/bin/env python3
"""
trade_reproducibility.py -- manuscript Fig 3 ("trade_reproducibility"): the operating-point
...
OUTPUTS
  manuscript/figures/trade_reproducibility.{png,pdf}   (PNG 300 dpi, PDF fonttype 42)
  manuscript/figures/trade_reproducibility.caption.md
  results/figures/manuscript/trade_reproducibility.tsv               (panels a + b)
  results/figures/manuscript/trade_reproducibility_reproducibility.tsv (panel c)
  results/figures/manuscript/trade_reproducibility_compute.tsv         (panel d)
  results/figures/manuscript/trade_reproducibility.png                 (copy)

RUN
  cd /mnt/ssd1/Projects/PeakATail_wd
  export LC_ALL=C            # REQUIRED (machine locale tr_TR corrupts sort/bedtools)
  python3 scripts/manuscript_figures/trade_reproducibility.py
...
footer = (
    ...
    "(unrunnable on this BAM, R1 = 28 bp); SCAPTURE has mouse 1 only. "
    ...
    "Every plotted value: results/figures/manuscript/trade_reproducibility{,_reproducibility,_compute}.tsv."
)
...
k10 = {ds: sweep[(sweep.dataset == ds) & (sweep.min_molecules == 10)].iloc[0]
       for ds in ("pbmc", "mouse1", "mouse2")}
cap_md = f"""# Fig 3 — `trade_reproducibility` caption (generated by `scripts/manuscript_figures/trade_reproducibility.py`)

**Index paragraph.** `trade_reproducibility` — Fig 3: the reliability trade surface, its window
...
measurements are audit rows in `trade_reproducibility_compute.tsv` and are not plotted.
...
columns of `results/figures/manuscript/trade_reproducibility.tsv`,
`trade_reproducibility_reproducibility.tsv` and `trade_reproducibility_compute.tsv`.
"""
