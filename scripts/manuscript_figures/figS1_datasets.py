...
OUTPUTS
  manuscript/figures/figS1_datasets.{png,pdf}  (PNG 300 dpi, PDF fonttype 42)
  manuscript/figures/figS1_datasets.caption.md
...
# ---------------------------------------------------------------------------
fig = plt.figure(figsize=(8.9, 7.6))
gsT = fig.add_gridspec(1, 1, left=0.015, right=0.985, top=0.955, bottom=0.520)
gsB = fig.add_gridspec(1, 1, left=0.130, right=0.985, top=0.430, bottom=0.225)
...
caption = (
    "Fig S1 | The datasets behind every headline number, with the poly(A) clip rate -- the "
...
assert _cap.count("\n") + 1 <= 9, _cap.count("\n") + 1
fig.text(0.015, 0.012, _cap, fontsize=5.8, color=INK, va="bottom", ha="left", linespacing=1.35)
