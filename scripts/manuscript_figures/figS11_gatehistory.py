...
OUTPUTS
  manuscript/figures/figS11_gatehistory.{png,pdf}  (PNG 300 dpi, PDF fonttype 42)
  manuscript/figures/figS11_gatehistory.caption.md
...
# figure
# ---------------------------------------------------------------------------
fig = plt.figure(figsize=(8.7, 11.4))
gsA = fig.add_gridspec(1, 1, left=0.025, right=0.985, top=0.965, bottom=0.630)
gsB = fig.add_gridspec(1, 1, left=0.025, right=0.985, top=0.575, bottom=0.135)
...
caption = (
    ...
    "decimals). Every event and row: results/figures/manuscript/figS11_gatehistory*.tsv."
)
_cap = textwrap.fill(caption, 168)
_n_lines = _cap.count("\n") + 1
assert _n_lines <= 15, f"caption is {_n_lines} wrapped lines"
fig.text(0.025, 0.010, _cap, fontsize=5.8, color=INK, va="bottom", ha="left", linespacing=1.33)
...
# sidecar caption
# ---------------------------------------------------------------------------
cap_md = f"""# Fig S11 — `figS11_gatehistory` caption (generated by `scripts/manuscript_figures/figS11_gatehistory.py`)
